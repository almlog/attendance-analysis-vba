...
' 残業一覧シートから部門別残業時間を集計
    ...
    ' ★★★ LINEWORKS通知ボタンを勤怠情報分析結果シートに追加 ★★★
    Call Add_LineWorks_Button_To_Summary
    
    ' ★★★ 矛盾レコードを違反者シートに統合 ★★★
    Dim contradictionCount As Long
    contradictionCount = 矛盾レコードを違反者シートに統合()
    
' ★★★ 勤怠入力漏れ一覧から純粋な漏れ件数を取得（矛盾を除く） ★★★
    Dim missingSheet As Worksheet
    Dim pureMissingCount As Long
    pureMissingCount = 0
    
    On Error Resume Next
    Set missingSheet = ThisWorkbook.Worksheets("勤怠入力漏れ一覧")
    On Error GoTo 0
    ...
    If Not missingSheet Is Nothing Then
        ...
        totalMissingFromSheet = missingSheet.Range("J2").Value  ' 総件数
        contradictionFromSheet = missingSheet.Range("J7").Value  ' 矛盾件数
        
        ' 純粋な勤怠入力漏れ = 総件数 - 矛盾件数
        pureMissingCount = totalMissingFromSheet - contradictionFromSheet
        
        Debug.Print "[DEBUG] 純粋な勤怠入力漏れ: " & pureMissingCount
    End If
    
    ' ★★★ 純粋な休憩時間違反数（矛盾を除く） ★★★
    Dim pureViolationCount As Long
    pureViolationCount = violationCount  ' 元の休憩時間違反数（矛盾統合前）
    ...
    If Not summarySheet Is Nothing Then
        ...
        Debug.Print "[INFO] 勤怠情報分析結果シートの統計を最終更新しました: 総違反=" & totalViolationCount
    End If
    
    MsgBox "休憩時間・残業時間チェックが完了しました。" & vbCrLf & _
           "総レコード数: " & totalCount & vbCrLf & _
           "処理対象レコード数: " & processedCount & vbCrLf & _
           "休憩時間違反: " & pureViolationCount & "件" & vbCrLf & _
           "届出違反（半休矛盾）: " & contradictionCount & "件" & vbCrLf & _
           "勤怠入力漏れ: " & pureMissingCount & "件" & vbCrLf & _
...
Sub CSV読み込みシート作成()
    ...
    With mainSheet
        ...
        .Range("A17").Value = "・LINEWORKS通知ボタンを押下すると、勤怠入力漏れ一覧シートの内容が専用チャンネルに通知される"
        .Range("A18").Value = "・未入力日数に応じて以下のように分類して表示されます："
        .Range("A19").Value = "  [緊急] 5日以上未入力 / [要注意] 3-4日未入力 / [確認] 1-2日未入力"
        ...
        .Range("A22").Value = "機能追加や不具合修正のご要望は下記までご連絡ください："
        .Range("A23").Value = "  連絡先: suzuki.shunpei@altx.co.jp"
        .Range("A23").Font.Bold = True
        .Range("A23").Font.Size = 11
        .Range("A23").Font.Color = RGB(0, 102, 204)
        
        .Range("A24").Value = "※ご連絡の際は、具体的な内容や動作環境をお知らせください"
...
' =====================================================
    ...
    ' ★★★ 稼働率計算を追加 ★★★
    On Error Resume Next
    Debug.Print "稼働率計算を開始します..."
    Call 部門別稼働率集計
    If Err.Number <> 0 Then
        Debug.Print "稼働率計算でエラー: " & Err.Description
        Err.Clear
    End If
    On Error GoTo 0
    
    MsgBox "部門別残業時間・定時退社率・稼働率の集計が完了しました。", _
           vbInformation, "集計完了"
End Sub

' *************************************************************
' 関数名: Add_LineWorks_Button_To_Summary
' 目的: 勤怠情報分析結果シートにLINEWORKS通知ボタンを追加
' 呼び出し元: 休憩時間チェック処理の最後
' 作成日: 2025-10-19
' 修正日: 2025-11-14（ボタン位置をD7に変更）
' 備考: D7セル付近に配置
' *************************************************************
Public Sub Add_LineWorks_Button_To_Summary()
    ...
    If summarySheet Is Nothing Then
        Debug.Print "[INFO] 勤怠情報分析結果シートが見つかりません"
        Exit Sub
    End If
    
    ' 既存のLINEWORKS通知ボタンを削除（重複防止）
    Dim btn As Button
    On Error Resume Next
    ...
    With summarySheet
        ...
        buttonWidth = 150  ' ボタンの幅（ピクセル）
        buttonHeight = 30  ' ボタンの高さ（ピクセル）
        ...
        With newButton
            .OnAction = "SendNotificationToLineWorks"  ' Module8_Notification.bas の関数
            .Caption = "LINEWORKS通知"
            .Font.Size = 11
            .Font.Bold = True
        End With
    ...
    Debug.Print "[INFO] 勤怠情報分析結果シートにLINEWORKS通知ボタンを追加しました（D7）"
    
    Exit Sub
    
ErrorHandler:
    Debug.Print "[ERROR] LINEWORKS通知ボタン追加エラー: " & Err.Description
End Sub

' *************************************************************
' 関数名: 部門別稼働率集計
' 目的: グループ(部門)ごとの稼働率を計算し、分析結果シートに出力
' 作成日: 2025-11-14
' 備考: 1日7.5時間、営業日100%を基準として稼働率を算出
' *************************************************************
Sub 部門別稼働率集計()
    Dim wsCSVData As Worksheet
    Dim wsSummary As Worksheet
    ...
    If wsSummary Is Nothing Then
        MsgBox "勤怠情報分析結果シートが見つかりません。", vbExclamation
        Exit Sub
    End If
    On Error GoTo 0
    ...
    ' 必須列の確認
    If 部門Col = 0 Or 実働時間Col = 0 Or 日付Col = 0 Or 社員番号Col = 0 Then
        MsgBox "必要な列（部門、実働時間、日付、社員番号）が見つかりません。", vbExclamation
        Exit Sub
    End If
    ...
    ' 標準稼働時間（分単位）
    Const STANDARD_DAILY_MINUTES As Double = 450 ' 7.5時間 = 450分
    ...
    For i = 2 To lastRow
        ...
        If Not dict.Exists(deptCode) Then
            ...
            ' 配列要素: (0)合計実働分, (1)営業日数, (2)社員Dict, (3)日付Dict
        End If
        ...
        ' 有休や特別休暇を除外するかの判定
        Dim 届出内容 As String
        Dim 稼働時間としてカウント As Boolean
        稼働時間としてカウント = True
        
        If 届出内容Col > 0 Then
            届出内容 = Trim(wsCSVData.Cells(i, 届出内容Col).Value)
            
            ' 除外する届出内容のパターン
            ' ※要件に応じて調整してください
            If InStr(届出内容, "有休") > 0 Or _
               InStr(届出内容, "特別休暇") > 0 Or _
               InStr(届出内容, "代休") > 0 Then
                稼働時間としてカウント = False
            End If
        End If
        
        ' 稼働時間として集計する場合のみ加算
        If 稼働時間としてカウント And workingMinutes > 0 Then
            ' 実働時間を加算
            deptData(0) = deptData(0) + workingMinutes
            
            ' 日付を営業日としてカウント
            Dim 日付Value As String
            日付Value = wsCSVData.Cells(i, 日付Col).Value
            
            Dim 日付Dict As Object
            Set 日付Dict = deptData(3)
            
            If Not 日付Dict.Exists(日付Value) Then
                日付Dict.Add 日付Value, True
                deptData(1) = deptData(1) + 1 ' 営業日数を増加
            End If
            
            ' 社員をカウント
            Dim 社員番号 As String
            社員番号 = wsCSVData.Cells(i, 社員番号Col).Value
            
            Dim 社員Dict As Object
            Set 社員Dict = deptData(2)
            
            If Not 社員Dict.Exists(社員番号) Then
                社員Dict.Add 社員番号, True
            End If
        End If
...
NextRow:
    ...
    ' ヘッダーを設定
    wsSummary.Cells(headerRow, 1).Value = "部署"
    wsSummary.Cells(headerRow, 2).Value = "営業日数"
    wsSummary.Cells(headerRow, 3).Value = "合計実稼働時間"
    wsSummary.Cells(headerRow, 4).Value = "基準稼働時間"
    wsSummary.Cells(headerRow, 5).Value = "稼働率(%)"
    wsSummary.Cells(headerRow, 6).Value = "対象人数"
    ...
    For Each dept In dict.keys
        ...
        合計実働分 = deptData(0)
        営業日数 = deptData(1)
        社員数 = deptData(2).count
        
        ' ★★★ 修正: 基準稼働時間の計算 ★★★
        ' 基準稼働時間 = 標準日次時間 × 営業日数 × 人数
        Dim 基準稼働分 As Double
        基準稼働分 = STANDARD_DAILY_MINUTES * 営業日数 * 社員数
        ...
        ' 全体集計に加算
        totalWorkMinutesAll = totalWorkMinutesAll + 合計実働分
        totalWorkDaysAll = totalWorkDaysAll + 営業日数
        ...
        wsSummary.Cells(summaryRow, 2).Value = 営業日数
        wsSummary.Cells(summaryRow, 3).Value = MinutesToTime(合計実働分)
        wsSummary.Cells(summaryRow, 4).Value = MinutesToTime(基準稼働分)
        wsSummary.Cells(summaryRow, 5).Value = Format(稼働率, "0.0") & "%"
        wsSummary.Cells(summaryRow, 6).Value = 社員数

        summaryRow = summaryRow + 1
    Next dept
    
    ' 合計行を出力
    Dim 全体基準稼働分 As Double
    全体基準稼働分 = STANDARD_DAILY_MINUTES * totalWorkDaysAll
    
    Dim 全体稼働率 As Double
    If 全体基準稼働分 > 0 Then
        全体稼働率 = (totalWorkMinutesAll / 全体基準稼働分) * 100
    Else
        全体稼働率 = 0
    End If
    ...
    wsSummary.Cells(summaryRow, 4).Value = MinutesToTime(全体基準稼働分)
    wsSummary.Cells(summaryRow, 5).Value = Format(全体稼働率, "0.0") & "%"
    ...
    Debug.Print "稼働率集計が完了しました"
End Sub

' *************************************************************
' 関数名: 矛盾レコードを違反者シートに統合
' 目的: 勤怠入力漏れ一覧から矛盾レコードを読み取り、
'       休憩時間チェック_違反者シートに追加する
' 戻り値: 矛盾レコード数
' 呼び出し元: 休憩時間チェックのメイン処理の最後
' 作成日: 2025-11-17
' *************************************************************
Public Function 矛盾レコードを違反者シートに統合() As Long
    On Error GoTo ErrorHandler
    ...
    If lastRowMissing <= 1 Then
        Debug.Print "[INFO] 勤怠入力漏れ一覧に矛盾レコードがありません"
        矛盾レコードを違反者シートに統合 = 0
        Exit Function
    End If
    ...
    ' 「違反はありません」メッセージがある場合は削除
    If violationSheet.Cells(2, 1).Value = "休憩時間違反はありません。" Then
    ...
    For i = 2 To lastRowMissing
        ...
        If missingSheet.Cells(i, 1).Interior.Color = COLOR_CONTRADICTION Then
            ' 矛盾レコードを違反者シートにコピー
            With violationSheet
                .Cells(nextRowViolation, 1).Value = missingSheet.Cells(i, 1).Value  ' 社員番号
                .Cells(nextRowViolation, 2).Value = missingSheet.Cells(i, 2).Value  ' 氏名
                .Cells(nextRowViolation, 3).Value = ""                                ' 部門（空白）
                .Cells(nextRowViolation, 4).Value = missingSheet.Cells(i, 3).Value  ' 日付
                .Cells(nextRowViolation, 5).Value = missingSheet.Cells(i, 8).Value  ' 出勤時刻
                .Cells(nextRowViolation, 6).Value = missingSheet.Cells(i, 9).Value  ' 退勤時刻
                .Cells(nextRowViolation, 7).Value = ""                                ' 実働時間（空白）
                .Cells(nextRowViolation, 8).Value = ""                                ' 休憩時間（空白）
                .Cells(nextRowViolation, 9).Value = ""                                ' 必要休憩時間（空白）
                .Cells(nextRowViolation, 10).Value = ""                               ' 休憩不足（空白）
                .Cells(nextRowViolation, 11).Value = missingSheet.Cells(i, 7).Value ' コメント（矛盾内容）
                
                ' 時刻列の書式設定
                .Cells(nextRowViolation, 5).NumberFormat = "h:mm"
                .Cells(nextRowViolation, 6).NumberFormat = "h:mm"
                
                ' オレンジ色でハイライト（矛盾レコードであることを示す）
    ...
    Debug.Print "[INFO] 矛盾レコード " & contradictionCount & " 件を違反者シートに統合しました"
    ...
    If contradictionCount > 0 Then
        ...
        If Not summarySheet Is Nothing Then
            ...
            Debug.Print "[INFO] 勤怠情報分析結果シートの統計を更新しました"
        End If
    End If
    
    ' 矛盾レコード数を返す
    矛盾レコードを違反者シートに統合 = contradictionCount
    
    Exit Function
    
ErrorHandler:
    Debug.Print "[ERROR] 矛盾レコード統合エラー: " & Err.Description
    矛盾レコードを違反者シートに統合 = 0
End Function
